...
F3 (Announ.) broadcasts the badge's node info and telemetry immediately, so a nearby device
...
A direct message asks for a delivery acknowledgement. When it comes back, the sent bubble gets
a check mark.

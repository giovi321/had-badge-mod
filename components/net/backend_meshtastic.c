...
static const setting_t MESH_SCHEMA[] = {
    ...
    {.key = "mesh_info_int", .type = SET_INT, .def = "300", .label = "Announce me every (s)", .group = "Radio",
     .minv = 30, .maxv = 3600, .has_min = true, .has_max = true,
     .help = "How often this node tells the others it exists"},
...
void mtb_on_frame(const uint8_t *frame, int len, float rssi, float snr, uint32_t now)
{
    ...
    net_message_t m = {0};
    m.from_id = hdr.from; m.channel = (uint8_t)chan;
